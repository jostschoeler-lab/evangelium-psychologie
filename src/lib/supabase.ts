...
if (!SUPABASE_URL) throw new Error("Fehler: VITE_SUPABASE_URL fehlt oder ist leer.")
if (!/^https:\/\/[a-z0-9.-]+\.supabase\.co$/.test(SUPABASE_URL)) {
  throw new Error(`Fehler: Supabase URL ungültig -> "${SUPABASE_URL}"`)
}
if (!SUPABASE_KEY) throw new Error("Fehler: VITE_SUPABASE_ANON_KEY fehlt oder ist leer.")
